...
export default function MortgageForm() {
  ...
  const [form, setForm] = useState<Partial<MortgageInputs>>({
    propertyType: "apartment", propertyStatus: "ready", nationality: "expat",
    employmentType: "salaried", rateType: "fixed", fixedPeriodYears: 3,
    tenureYears: 20, emirate: "Dubai", existingLiabilities: 0, downPayment: 0,
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const params = new URLSearchParams(Object.entries(form).map(([k, v]) => [k, String(v)]));
    router.push(`/results?${params.toString()}`);
  };

  const ltvHint = form.propertyStatus === "off-plan"
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-6 md:p-8 space-y-6">
      <section>
        <h3 className="font-semibold text-lg mb-4" style={{ color: "#1e3a5f" }}>Property Details</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Property Price (AED)</label>
            <input type="number" required min={300000} placeholder="e.g. 1500000"
          ...
          <div>
            ...
            <input type="number" required min={0} placeholder="e.g. 300000"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            ...
            <select className="w-full border border-gray-300 rounded-lg px-3 py-2"
              value={form.propertyStatus} onChange={(e) => set("propertyStatus", e.target.value)}>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" required min={5000} placeholder="e.g. 25000"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            ...
            <input type="number" min={0} defaultValue={0} placeholder="e.g. 5000"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <select className="w-full border border-gray-300 rounded-lg px-3 py-2"
              value={form.rateType} onChange={(e) => set("rateType", e.target.value)}>
          ...
          {form.rateType === "fixed" && (
            <div>
              ...
              <select className="w-full border border-gray-300 rounded-lg px-3 py-2"
                value={form.fixedPeriodYears} onChange={(e) => set("fixedPeriodYears", Number(e.target.value))}>
          ...
          <div>
            ...
            <select className="w-full border border-gray-300 rounded-lg px-3 py-2"
              value={form.tenureYears} onChange={(e) => set("tenureYears", Number(e.target.value))}>
